...
class Trainer:
    def __init__(self):
        ...
        if args.is_poison:
            ...
            if args.attack_mode == 'DBA':
                adversary_index = list(range(args.dba_trigger_num)) * int(args.number_of_adversaries / args.dba_trigger_num)
                ...
                adversary_index = list(range(args.multi_objective_num)) * int(args.number_of_adversaries / args.multi_objective_num)
                assert len(adversary_list) == len(adversary_index)
                for idx, ind in zip(adversary_list, adversary_index):
                    clients[idx].malicious = True
                    clients[idx].adversarial_index = ind
    ...
    def train(self):
        ...
        if args.is_poison:
            end_round = start_round + args.retrain_rounds
        ...
        for _ in range(start_round, end_round):
            ...
            self.server.train_and_aggregate(global_lr=1)
